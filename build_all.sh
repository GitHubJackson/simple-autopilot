...
echo "=== 3. 编译 System Monitor (系统监视器) ==="
...
make install
if [ $? -eq 0 ]; then
    echo "System Monitor 编译成功！"
else
    echo "System Monitor 编译失败！"
    exit 1
fi
cd ../..

echo "=== 4. 编译 Control 模块 (依赖中间件 & Common Msgs) ==="
...
echo "=== 5. 编译 Visualizer 模块 (依赖中间件 & Common Msgs) ==="
...
echo ""
echo "=============================================="
echo "   编译完成！使用以下命令查看系统状态："
echo "=============================================="
echo "   ./autopilot status"
echo ""
echo "请在三个不同的终端中分别运行："
echo "1. [中间件监视器] (已升级为 autopilot status)"
echo "   ./autopilot status"
echo ""
echo "2. [控制模块] (生产者)"
